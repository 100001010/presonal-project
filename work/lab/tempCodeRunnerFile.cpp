...
class Checker:public Gene{
    public:
        Checker():Gene(checker_number){
            int checkpoint_count_max=stand_number;
            for(int &i:gene){
                i=rand()%checkpoint_count_max+1;
                checkpoint_count_max-=i;
            }
            gene[2]+=checkpoint_count_max;
        }
};
