...
class Time{
    public:
        ...
        int time_to_sec(){
            return ((((y*12+m)*32+d)*24+h)*60+m)+s;
        }
...
void unify(queue <Plate_data> acelin,queue <Plate_data> aaeon,queue <Plate_data> dvr,queue <Final_plate_data> &final,set <string> &dvr_white_list){
    ...
    while(!dvr.empty()){
        ...
        if(!aaeon.empty()&&aaeon.front().plate[0]==dvr.front().plate[0]){
            cout<<aaeon.front().plate[0]<<" 466 add final"<<endl<<aaeon.front().vague;
            
            final.back().aaeon=aaeon.front();
            //cout<<final.back().aaeon.vague;
            aaeon.pop();    
        }else if(!aaeon.empty()&&editdistance(aaeon.front().plate[0],dvr.front().plate[0])<=2){
        ...
        if(!acelin.empty()&&acelin.front().plate[0]==dvr.front().plate[0]){
            ...
            acelin.pop();    
        }else if(!acelin.empty()&&editdistance(acelin.front().plate[0],dvr.front().plate[0])<=2){
            final.back().acelin=acelin.front();
...
int main(){
    ...
    push_aaeon_data(aaeon_data,dvr_white_list);return 0;
    ...
    unify(acelin_data,aaeon_data,dvr_data,final_data,dvr_white_list);
